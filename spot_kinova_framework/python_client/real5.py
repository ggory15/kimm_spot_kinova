...
class ControlSuiteShell(cmd.Cmd):
    ...
    def do_qrplant1pick_fromtray(self, arg):
        ...
        goal.target_pose.position.x    =  0.02 + 0.025 
        goal.target_pose.position.y    =  0.015+0.005 
        goal.target_pose.position.z    =  self.gripper_length - 0.05 +0.01
        goal.target_pose.orientation.x =  0.0
        goal.target_pose.orientation.y =  0.0
        goal.target_pose.orientation.z =  0.0
        goal.target_pose.orientation.w =  1.0        
    ...
    def do_qrtable1plantapproach(self, arg):
        ...
        x_qr2plantgrasp       = 0.085
        x_offset_before_place = 0.020
        y_qr2table1           = 0.22+0.005
        z_qr2plantgrasp       =-0.085
    ...
    def do_qrplant2pick_fromtray(self, arg):
        ...
        goal.target_pose.position.x    =  0.02 + 0.02 
        goal.target_pose.position.y    = -0.125 
        goal.target_pose.position.z    =  self.gripper_length - 0.05 
        goal.target_pose.orientation.x =  0.0
        goal.target_pose.orientation.y =  0.0
        goal.target_pose.orientation.z =  0.0
    ...
    def do_qrtable2plantapproach(self, arg):
        ...
        goal.target_pose.position.x    =  0.085 + 0.015 + self.place_dx    #qr2place + plantesizebuffer + place_dx   0.05+0.1                         #distance btw qr and grasp position in x direction w.r.t marker
        goal.target_pose.position.y    =  0.212 - 0.14 + self.place_dy   #0.02                        #distance btw qr and box end in y direction w.r.t marker  
        goal.target_pose.position.z    =  self.gripper_length - 0.085 + self.place_dz   
        goal.target_pose.orientation.x =  0.0
        goal.target_pose.orientation.y =  0.0
        goal.target_pose.orientation.z =  0.0
        goal.target_pose.orientation.w =  1.0        
    ...
    def do_test_tray_pick(self, arg):    
        ...
        if (self.issucceed):
            self.do_armforward(100)  #go to tray qr position
        if (self.issucceed):
            self.do_armup(100)  #go to tray qr position
        if (self.issucceed):
            self.do_qr5save(arg)  #save qr5 pose
        if (self.issucceed):
            self.do_position(80)   #pick the tray              
        if (self.issucceed):
        ...
        if (self.issucceed):
            self.do_armback(250)    #pick up the object
        if (self.issucceed):
            self.do_prefold(arg)  #go to preready
    ...
    def do_test_plant1_tray_to_table(self, arg):    
        ...
        if (self.issucceed):
            self.do_preapproach(arg)  #go to prefold                       
        if (self.issucceed):
            self.do_armdown(100)  #save qr5 pose               
        if (self.issucceed):
            time.sleep(5) #wait for camera focusing
            self.do_qr4save(arg)  #save qr5 pose   
        if (self.issucceed):
            self.do_armup(150)  #save qr5 pose               
        if (self.issucceed):
            self.do_armright(50)  #save qr5 pose               
        if (self.issucceed):
            time.sleep(5) #wait for camera focusing
            self.do_qr5save(arg)  #save qr4 pose                             
        if (self.issucceed):
            self.do_position(50)   #pick the plant                              
        if (self.issucceed):
    ...
    def do_test_plant2_tray_to_table(self, arg):    
        ...
        if (self.issucceed):
            ...
            time.sleep(3) #wait for camera focusing
        if (self.issucceed):
            self.do_qr4save(arg)  #save qr4 pose
        if (self.issucceed):
            self.do_goto_tray_for_qrsave(arg);
            time.sleep(2) #wait for camera focusing
        if (self.issucceed):
        ...
        if (self.issucceed):
            self.do_position(50)   #pick the plant                              
        if (self.issucceed):
            self.do_qrplant2pick_fromtray(arg) #pick the plant2 w.r.t marker 13
        if (self.issucceed):
            self.do_position(70)   #pick the plant                      
        if (self.issucceed):
            self.do_armup(100)    #pick up the object
        if (self.issucceed):
            self.do_armleft(100)    #pick up the object
        if (self.issucceed):

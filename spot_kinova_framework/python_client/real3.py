...
class ControlSuiteShell(cmd.Cmd):
    ...
    def do_qrpick(self, arg):
        ...
        goal.target_pose.position.x    =  0.048
        goal.target_pose.position.y    = -0.109        
    ...
    def do_gotobox(self, arg):
        ...
        goal.target_poses = PoseArray()
        pose_se = Pose()
        pose_se.position.x = 0.00
        pose_se.position.y = -0.2
    ...
    def do_qrboxpick(self, arg):
        ...
        goal.target_pose.position.x    =  0.18
        goal.target_pose.position.y    =  0.0
        goal.target_pose.position.z    =  self.gripper_length + 0.07
        goal.target_pose.orientation.x =  0.0
        goal.target_pose.orientation.y =  0.0
        goal.target_pose.orientation.z =  0.0
        goal.target_pose.orientation.w =  1.0
    ...
    def do_mech_test(self, arg):    
        if (self.issucceed):
            self.do_prefold(arg)   #fold arm for mech recognition
        if (self.issucceed):
